...
describe('resource allowance', () => {
  ...
  test('approve: only required params', async () => {
    const responsePromise = client.blockchain.allowance.approve({ amount: 'amount', spender: 'spender' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('approve: required and optional params', async () => {
    const response = await client.blockchain.allowance.approve({ amount: 'amount', spender: 'spender' });
  });
});
